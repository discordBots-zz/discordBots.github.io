...
''                 'Note: !kamadan+/- are the respective on/off toggles for the ad channel (when used w/o arguments)'\n\
...
bot.on("message",function(message)
{
  ...
  */
  ...
  if(tbText.charAt(0) === "!" || tbText.charAt(0) === "！")
  {
    ...
    if ((tbText.match(/^激战/gi) || tbText.match(/^激戰/gi) || tbText.match(/^guildwars*?/gi))){
      ...
      if (tbText === ""){
        if (tAuthor.hasRole(roleIDChinese)){
            bot.addMemberToRole(tAuthor, roleIDKamadanChinese);
        } else if (tAuthor.hasRole(roleIDOtherLang)) {
            bot.addMemberToRole(tAuthor, roleIDKamadanFrgn);
        }
      }
      ...
      UserDB.discordUsers.find({userID:message.author.id}, function(err, users){
          ...
          OpenCC.traditionalToSimplified(tbText).then(function(tResult){
              ...
              if (addArray.length > 0){
                ...
                for (var q=0;q<addArray.length;q++){
                  var tempItemName = addArray[q];
                  var addThisArray = [tAuthorID];
                  UserDB.adTable.update({itemName:tempItemName},{ $addToSet: { interestedParty: { $each: addThisArray}}},{ upsert:true });
                }
              }
      ...
      if (tbText === ""){
        if (tAuthor.hasRole(roleIDChinese)){
            bot.removeMemberFromRole(tAuthor, roleIDKamadanChinese);
        } else if (tAuthor.hasRole(roleIDOtherLang)) {
            bot.removeMemberFromRole(tAuthor, roleIDKamadanFrgn);
        }
      }
      ...
      OpenCC.traditionalToSimplified(tbText).then(function(tResult){
          ...
          if (subArray.length > TrackingLimit) {
            subArray.splice(TrackingLimit, subArray.length - TrackingLimit);
          }
          if (subArray.length > 0) { //
            UserDB.discordUsers.find({userID:message.author.id}, function(err, users){
              if ((!err) && (users.length) && (users[0]["trackedItems"])) {
                ...
                for (var q=0;q<subArray.length;q++){
                  var tempItemName = subArray[q];
                  var subThisArray = [tAuthorID];
                  UserDB.adTable.update({itemName:tempItemName},{ $pull: { interestedParty: { $in: subThisArray}}});
                }
              } else {
      ...
      var tAuthorArray = [ttAuthor.id];
      UserDB.discordUsers.update({userID:message.author.id}, {$set:{userName:message.author.name, trackedItems:[]}}, {upsert:true}, function (err, doc) {
            printUpdatedTrackingTable(message);
      });
      UserDB.adTable.update({},{ $pull: { interestedParty: { $in: tAuthorArray}}},{ multi: true });
    ...
    } else if ((tbText==="showAll") && (message.author.hasRole(roleIDAdmin))){
      clearStorage();
      var tAuthor = message.author;
      var tOutputs = "";
      var tOutputs2 = "";
      UserDB.discordUsers.find(function(err, users){
        if (err || !users.length){
          bot.sendMessage(tAuthor, "用户数据库: [空]");
        } else{
      ...
      UserDB.adTable.find(function(err, users){
        if (err || !users.length){
          bot.sendMessage(tAuthor, "广告数据库: [空]");
        } else{
          for (i=0;i<users.length;i++){
            tOutputs2 = tOutputs2 + JSON.stringify(users[i]) + "\n";
          }
          bot.sendMessage(tAuthor, tOutputs2);
        }
      });
      ...
      if (tTargetUser){
        if (tTargetUser.hasRole(roleIDChinese)) {
          bot.sendMessage(tTargetUser, Greetings).then(function(stuff){
        ...
        if (tAuthor.hasRole(roleIDChinese)) {
          bot.sendMessage(tAuthor, Greetings).then(function(stuff){
      ...
      UserDB.discordUsers.find({userID:tAuthorID}, function(err, users){
        if ((err) || !(users.length) || !(users[0]["clientID"]) || !(users[0]["clientSecret"]) || (users[0]["clientID"] == "") || (users[0]["clientSecret"] == "")) {
            if (!message.channel.isPrivate) {bot.deleteMessage(message);}
            if (tAuthor.hasRole(roleIDChinese)) {
              bot.sendMessage(tAuthor, "照链接内说明 (http://ally.boards.net/thread/12/) 获取一帐号名及密码, 再用以下指令刷新档案: !翻译安装 帐号名, 密码\n");
            } else if (tAuthor.hasRole(roleIDOtherLang)) {
            ...
            if (((textSource === "error")||(textDest==="error")) && !recordExists){
                ...
                if (!message.channel.isPrivate) {bot.deleteMessage(message);}
                if (tAuthor.hasRole(roleIDChinese)) {
                  bot.sendMessage(tAuthor, "[失败：重新输入语言选择: 中简繁德英西法葡俄]");
                } else if (tAuthor.hasRole(roleIDOtherLang)) {
              ...
              if ((textSource === "error")||(textDest==="error")){
                ...
                UserDB.discordUsers.update({userID:tAuthorID}, {$set:{sourceLang:textSource, destLang:textDest}}, {upsert:true}, function (err, doc) {
                  if (!err){
                    if (tAuthor.hasRole(roleIDChinese)) {
                      bot.sendMessage(tAuthor, "[已保留语言选择: "+langCode(textSource, 1)+"-"+langCode(textDest, 1)+". 若非更换选择, 此后无需指定语言 | 例: !!早上好 或 !!!早上好]\n");
                    } else if (tAuthor.hasRole(roleIDOtherLang)) {
                    ...
                    if (tAuthor.hasRole(roleIDChinese)) {
                      bot.sendMessage(tAuthor, "[提示: 无法保留语言选择]");
                    } else if (tAuthor.hasRole(roleIDOtherLang)) {
                ...
                if ((textSource===textDest) || ((textSource === "zh-CHT") && (textDest === "zh-CHS"))) {
                    ...
                    if (!(tResult.match(/^[\s.。，；;,?？!！]*$/gi))){
                      if (isPrivate || sendDirect){
                        if (tAuthor.hasRole(roleIDChinese)) {
                          bot.sendMessage(tAuthor, "繁-简"+": "+tResult);
                        } else if (tAuthor.hasRole(roleIDOtherLang)) {
                  ...
                  OpenCC.simplifiedToTraditional(tResult).then(function(TTResult){
                      ...
                      if (!(TTResult.match(/^[\s.。，；;,?？!！]*$/gi))){
                        if (isPrivate || sendDirect){
                          if (tAuthor.hasRole(roleIDChinese)) {
                            bot.sendMessage(tAuthor, "简-繁"+": "+TTResult);
                          } else if (tAuthor.hasRole(roleIDOtherLang)) {
      ...
      if (setupArray.length < 2){
          if (!message.channel.isPrivate) {bot.deleteMessage(message);}
          if (tAuthor.hasRole(roleIDChinese)) {
            bot.sendMessage(tAuthor, "安装出错: 参数缺失 | 以逗号隔开 帐号名 及 密码");
          } else if (tAuthor.hasRole(roleIDOtherLang)) {
        ...
        UserDB.discordUsers.update({userID:tAuthorID},{ $set:{clientID:storeID, clientSecret:storeSecret}},{upsert:true}, function(err,doc){
            if (!message.channel.isPrivate) {bot.deleteMessage(message);}
            if (tAuthor.hasRole(roleIDChinese)) {
              bot.sendMessage(tAuthor, "[安装完毕: 账号名: "+storeID + " | 密码: "+storeSecret+"]");
            } else if (tAuthor.hasRole(roleIDOtherLang)) {
      ...
      tuling.send({
        ...
        info: tbText,
          ...
          switch(result.code) {
              case 40001:
                  styledOutput = "[参数错误 -> 重发]";
                  break;
              case 40002:
                  styledOutput = "[传送出错 -> 重发]";
                  break;
              case 40004:
                  styledOutput = "[已达对话次数上线，明日再见]";
                  break;
              case 40007:
                  styledOutput = "[数据格式异常 -> 重发]";
                  break;
              case 100000:
      ...
      }).catch(function(err){bot.sendMessage(message, "[系统出错 -> 重发]");});
    }
...
bot.on("presence", function(userPast,userPresent) {
  ...
  if ((userPast.status === 'offline') && (userPresent.status === 'online')) {
    if (userPresent.hasRole(roleIDChinese)){
        bot.sendMessage(userPresent, Greetings).then(function(stuff){
          displayTrackedItems(userPresent);
        }).then(function(stuff){scrapeDailyInfo(userPresent, '中文', "", "");});
...
*/
  ...
  UserDB.discordUsers.find({userID:message.author.id}, function(err, users){
    if (err || !users.length){
      ...
      var trackingArray = [];
      var testCondition = users[0]["trackedItems"];
      if (testCondition && (testCondition != "") && (testCondition != [])){
...
function responseHandler(resolvable, messageChinese, messageForeign){
  if (resolvable.hasRole(roleIDChinese)){
    bot.sendMessage(resolvable, messageChinese);
  } else if (resolvable.hasRole(roleIDOtherLang)){
...
bot.loginWithToken("Bot MjAyMjk0NTg4NDQ5MzU3ODI0.Coaeag.eAET6Kzptp6JnnYKO6M7K-AXAUs");
