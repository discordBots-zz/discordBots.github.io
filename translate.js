...
module.exports = {
  ...
  translate: function(data, kAd = true){
    if (kAd) {data=data.replace(/\d\s*/gi,'');}

    data=data.replace(/Extra Luck Bonus/gi, '份外运气奖 (获以往两倍的幸运及不幸运分；获四倍以往的钥匙和开锁道具掉率)');
    data=data.replace(/Elonian Support Bonus/gi, '支援伊洛那奖 (获以往两倍的日戟和光明分)');
    data=data.replace(/Zaishen Bounty Bonus/gi, '战承悬赏奖 (战承悬赏任务奖品(铜战承币)翻倍)');
    data=data.replace(/Factions Elite Bonus/gi, '二章精英奖 (可从凯宁城进入深渊和尔果)');
    data=data.replace(/Northern Support Bonus/gi, '四章支援奖 (获以往两倍的阿苏拉, 戴尔狄摩, 黑檀先锋队, 和诺恩分)');

    data=data.replace(/Zaishen Mission Bonus/gi, '战承主线奖 (战承主线任务奖品(铜战承币)翻倍)');
    data=data.replace(/Pantheon Bonus/gi, '神殿奖 (可免费进入地下和灾难)');
    data=data.replace(/Faction Support Bonus\*?/gi, '二章支援奖 (上交分数时，获以往两倍的库兹柯和勒克森分)');
    data=data.replace(/Zaishen Vanquishing Bonus/gi, '战承清图奖 (	战承清图任务奖品(铜战承币)翻倍)');
    data=data.replace(/Random Arenas Bonus/gi, '随机竞技奖 (随机竞技场内获 以往两倍的巴萨泽和斗士分)');

    data=data.replace(/Guild Versus Guild Bonus/gi, '公会战奖 (公会战内获 以往两倍的巴萨泽和冠军分)');
    data=data.replace(/Competitive Mission Bonus/gi, '二章竞技奖 (翡翠矿场和杨木要塞内获 以往两倍的巴萨泽和帝国分)');
    data=data.replace(/Heroes' Ascent Bonus/gi, '英雄之路奖 (英雄之路内获 以往两倍的巴萨泽和英雄称号分)');
    data=data.replace(/Codex Arena Bonus/gi, 'Codex竞技奖 (Codex竞技场内获 以往两倍的巴萨泽和Codex称号分)');
    data=data.replace(/Alliance Battle Bonus/gi, '同盟战奖 (同盟战内获 以往两倍的巴萨泽和帝国分)');
  ...
  parseTranslate: function(data){
  	...
  	data=data.replace(/(^|[^A-Za-z])(AUTOMATONICS*?)(?=[^A-Za-z]|$)/gi, '$1高仑 [http://wiki.guildwars.com/wiki/Everlasting_Automatonic] [http://wiki.guildwars.com/wiki/Golem_Form] ');
